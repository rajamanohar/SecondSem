...
;(function($){
	$.contextMenu = {
		...
		createMenuItem: function(label,obj) {
			...
			var iconStyle = (o.icon)?'background-image:url('+o.icon+');':'';
                        var textStyle = (o.url!='')?'text-decoration:underline;':'';
			var $div = $('<div class="'+cmenu.itemClassName+' '+o.className+((o.disabled)?' '+cmenu.disabledItemClassName:'')+'" title="'+o.title+'"/>')
							// If the item is disabled, don't do anything when it is clicked
							.click(function(e){if(cmenu.isItemDisabled(this)){return false;}else{if(o.urlConfirmMsg==''||confirm(o.urlConfirmMsg)){if(o.url!=null&&o.url!=''){window.open(o.url,o.urlTarget,o.urlOptions);}}return o.onclick.call(cmenu.target,this,cmenu,e)};alert(o.url);})
			...
			var $idiv = $('<div class="'+cmenu.innerDivClassName+'" style="'+iconStyle+textStyle+'">'+label+'</div>');
			$div.append($idiv);
			return $div;
		},
		...
		getPosition: function(clickX,clickY,cmenu,e) {
			...
			if (cmenu.constrainToScreen) {
				...
				var maxRight = x+w-$w.scrollLeft() + 10;
				if (maxRight > ww) { x -= w; }
			}
			if (dir=="up") { 
				y -= h;
				if (y < 0) { y = $w.scrollTop(); }  
			}
			return {'x':x,'y':y};
		},
	...
	$.fn.contextMenu = function(menu,options,e,contextMenu) {
		if (contextMenu != null) {
			contextMenu.hide();
		}
